...
#define NEWLINE "\n"
	...
	namespace {
		...
		//DS		
		const char kJSONNativeMessage[] = "CallNativeWithJSON";
...
#define VALUE(val, def)       \
  if (val == def) {           \
	...
	ClientHandler::ClientHandler(Delegate* delegate,
		...
		resource_manager_->AddContentProvider("http://spp/", currentPath.c_str(), std::string(), 100, std::string());
		resource_manager_->AddDirectoryProvider("http://spp/", currentPath.c_str(), 100, std::string());
	...
	bool ClientHandler::OnPreKeyEvent(CefRefPtr<CefBrowser> browser,
		...
		//		if (_keyDown)
		//		{
		//			_keyDown(event.windows_key_code);
		//		}
		//	}
