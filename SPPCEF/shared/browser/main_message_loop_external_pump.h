...
namespace client {
	...
	public:
		...
		// application thread.
		static scoped_ptr<MainMessageLoopExternalPump> Create(std::function<void(void)> ExternUpdate = nullptr,
			std::function<void(uint8_t)> InKeyDown = nullptr, std::function<void(uint8_t)>  InKeyUp = nullptr);
