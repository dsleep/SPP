...
namespace client {
	...
	namespace {
		...
		{
		public:
			MainMessageLoopExternalPumpWin(std::function<void(void)> ExternUpdate = nullptr, std::function<void(uint8_t)> InKeyDown = nullptr, std::function<void(uint8_t)> InKeyUp = nullptr);
		...
		private:
			...
			std::function<void(void)> _externalUpdate;
			std::function<void(uint8_t)> _keyDown;
			std::function<void(uint8_t)> _keyUp;
		};

		MainMessageLoopExternalPumpWin::MainMessageLoopExternalPumpWin(std::function<void(void)> ExternUpdate,
			std::function<void(uint8_t)> InKeyDown, std::function<void(uint8_t)> InKeyUp)
			: timer_pending_(false), main_thread_target_(NULL), _externalUpdate(ExternUpdate), _keyDown(InKeyDown), _keyUp(InKeyUp) {
			...
			while (true)
			{
				...
				{					
					...
					else
					{
						...
						{
						case WM_KEYDOWN:
							if (_keyDown) _keyDown(static_cast<uint8_t>(msg.wParam));
							break;
						case WM_KEYUP:
							if (_keyUp) _keyUp(static_cast<uint8_t>(msg.wParam));
							break;
						}
	...
	// static
	scoped_ptr<MainMessageLoopExternalPump> MainMessageLoopExternalPump::Create(std::function<void(void)> ExternUpdate,
		std::function<void(uint8_t)> InKeyDown, std::function<void(uint8_t)>  InKeyUp) {
		return scoped_ptr<MainMessageLoopExternalPump>(
			new MainMessageLoopExternalPumpWin(ExternUpdate, InKeyDown, InKeyUp));
	}
