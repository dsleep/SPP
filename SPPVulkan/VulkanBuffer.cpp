...
namespace SPP
{
	...
	PerFrameStagingBuffer::Chunk::Chunk(uint32_t InSize, uint8_t currentFrameIdx) : size(InSize), frameIdx(currentFrameIdx)
	{
		...
		VkBufferUsageFlags usageFlags = VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
